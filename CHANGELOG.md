...
- **Oplock breaks are now acknowledged.** A batch oplock is the price of a durable handle (MS-SMB2 § 3.3.5.9.10), and while a client holds one, anyone else opening the file waits out the server's break timeout — about 35 s on both Samba and Windows. Breaks used to be logged and dropped. Acknowledging gives the oplock up and therefore the durability, which is the right trade: a resumable transfer is worth less than freezing somebody else's file operation for half a minute.
- **The client GUID is now stable for the life of the process**, as MS-SMB2 § 3.2.1.1 requires (it is a property of the *client*, not of a connection). It used to be regenerated on every NEGOTIATE, which reintroduced the client to the server on every connection.
- **`SESSION_SETUP` announces `PreviousSessionId` after a reconnect**, which is how a client says "this is me again". Without it the server holds the dead session's state until its own timeout.
...
- **An SMB2 ECHO keepalive, so "slow" and "dead" stop looking the same.** A response deadline on its own cannot tell them apart, so it has to be sized for the slowest healthy case, which makes it a poor detector of the dead one. That cuts both ways: a large write to a loaded spinning-disk NAS gets killed for being slow, while a genuinely dead session freezes a transfer for the full deadline. ECHO settles it — it touches no share, no handle, and no disk, so an answer means the server is processing requests, full stop.
  ...
  - **A request on a provably-alive connection now gets six times the response deadline** (3 minutes at the defaults) before being abandoned. Bounded, not forgiving: past that, "alive but still hasn't answered this one request" is a server-side stall that waiting cannot fix. The new `MetricsSnapshot::response_deadline_extensions` counts every request this rescued.
  - **Two unanswered probes declare the session dead** — at roughly 17 s rather than 30 s — and every waiter is told at once instead of discovering it one deadline at a time. One probe is not enough: on Samba (and the NAS firmware built on it) the process answering ECHO is the same one that may be blocked in a `pwrite` on a stalled disk.
  - **A `Watcher` on a dead session finally finds out.** CHANGE_NOTIFY is exempt from the response deadline by design — it waits for an event that may never come — so before this a dead server left a watcher waiting for hours on an event that could never arrive. It is deliberately *not* exempt from the keepalive.
  ...
  - Verified against real Samba in four session modes (guest, NTLM, mandatory signing, mandatory encryption): all four answer the probe as constructed, with the session id set and signed or encrypted to match the session.
- **`Connection::set_keepalive(Option<Duration>)`** (default `Some(5 s)`) — how much server silence, with a request on the wire, triggers a probe. `None` turns it off, which also turns off the deadline extension: with nothing refreshing the liveness clock, a fresh reading is luck rather than evidence, and a deadline extended on luck is the hang coming back.
- **`Error::ServerUnresponsive { silent_for, probes }`** — the socket is up and the server is answering nothing. Distinct from the three that already existed because they answer different questions: `Timeout` means *this* request went unanswered (the connection may be fine), `SendTimeout` means the request never reached the network so nothing about the server follows, `Disconnected` means the socket went away. Classifies as `ErrorKind::ConnectionLost`, the same as `Disconnected`, so existing reconnect paths pick it up unchanged, and reports `is_retryable() == true`.
- **`MetricsSnapshot::keepalive_probes_sent` / `keepalive_probes_skipped` / `keepalive_failures` / `response_deadline_extensions`.** `keepalive_probes_skipped` is the one to watch: a steady stream means the credit window is fully spent whenever the server goes quiet, so the connection has no liveness signal and slow-but-alive operations fall back to the plain deadline. Skips are never counted as failures — doing so would make the busiest healthy transfers the likeliest to be torn down.
