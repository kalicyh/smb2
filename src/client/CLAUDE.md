...
- CHANGE_NOTIFY is exempt from the *request's* deadline (`is_long_poll`) — it waits for an event that may never come. Add any new wait-for-an-event command there. It is bounded by the connection instead, in `await_long_poll`: the same `unresponsive_for` verdict on `quiet_for`'s clock, held to the response-deadline budget. Without that a `Watcher` on a dead session waits forever, and with the keepalive off it does exactly that, deliberately.
- ❌ **Anything holding a `WaiterGuard` must finish through `await_response`, never a bare `guard.recv()`.** `recv()` is an unbounded `oneshot` await — it walks past the deadline, the alive-extension, and the long-poll bound alike. `Watcher::next_events` did exactly that, so the one production consumer of the long-poll bound never reached it: against a real Samba with `smbd` suspended, 90 s of connection-wide silence and 15 consecutive unanswered probes left the watcher waiting (2026-08-02). Routed through `await_response` it is now told at 30.1 s, measured on the same rig.
...
The wait itself ends the same way every other one does: `next_events` hands its `WaiterGuard` to `Connection::await_response`, which routes CHANGE_NOTIFY to `await_long_poll` and ends it on connection-wide silence. See § Response deadline for why a bare `recv()` here is a bug rather than a shortcut.
