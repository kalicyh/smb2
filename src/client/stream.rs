...
pub async fn open_file_writer(
    ...
    let file_id = tree
        .open_file_for_write_bound(&mut conn, &normalized, generation, session_id)
        .await?;
...
pub async fn open_file_writer_exclusive(
    ...
    let session_id = conn.session_id();
    let file_id = tree
        .open_file_for_exclusive_create_bound(&mut conn, &normalized, generation, session_id)
        .await?;
    if conn.generation() != generation || conn.session_id() != session_id {
        return Err(Error::Disconnected);
    }
...
pub async fn open_file_writer_at(
    ...
    let session_id = conn.session_id();
    let file_id = tree
        .open_file_for_write_at_bound(&mut conn, &normalized, generation, session_id)
        .await?;
    if conn.generation() != generation || conn.session_id() != session_id {
        return Err(Error::Disconnected);
    }
...
pub async fn open_existing_file_writer_at(
    ...
    let (file_id, file_size, identity) = tree
        .open_existing_file_for_write_at(&mut conn, &normalized, generation, session_id)
        .await?;
    if conn.generation() != generation || conn.session_id() != session_id {
        return Err(Error::Disconnected);
    }
