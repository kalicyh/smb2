...
impl SmbClient {
    ...
    async fn resolve_dfs_redirect(
        ...
        for resolved in &resolved_list {
            ...
            match self.ensure_tree(&target_addr, &resolved.share).await {
                Ok(new_tree) => {
                    return Ok((new_tree, resolved.remaining_path.clone()));
                }
                Err(e) => {
                    debug!(
    ...
    pub async fn upload<'a>(
        ...
        if data.len() <= max_write {
            ...
            tree.write_file_compound(conn, normalized, data).await?;
            Ok(stream::FileUpload::new_done(tree, conn, data.len() as u64))
        } else {
            // Large file: open the file, let the caller drive chunks.
            let file_id = tree.open_file_for_write(conn, normalized).await?;
