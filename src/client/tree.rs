...
/// FileDispositionInformation class for SET_INFO (MS-FSCC 2.4.13).
...
impl Tree {
    ...
    ///
    /// Issues one six-operation stat compound per path and returns results in
    ...
    pub async fn stat_files(&self, conn: &mut Connection, paths: &[&str]) -> Vec<Result<FileInfo>> {
        ...
        let mut cleanup_handles: Vec<(FileId, u64, SessionId)> = Vec::new();
        ...
        for path in paths {
            ...
            let frames = match conn
                .execute_compound_bound(&ops, generation, session_id)
                .await
            {
                Ok(v) => v,
                Err(e) => {
            ...
            let parsed = self.parse_stat_batch_response(
                &responses,
                &mut cleanup_handles,
                generation,
                session_id,
        ...
        for &(file_id, generation, session_id) in &cleanup_handles {
            warn!(
    ...
    ///
    /// Expects exactly six sub-frames (CREATE, QUERY_INFO basic,
    /// QUERY_INFO standard, QUERY_INFO internal, QUERY_INFO volume, CLOSE).
    fn parse_stat_batch_response(
        &self,
        responses: &[crate::client::connection::Frame],
        cleanup_handles: &mut Vec<(FileId, u64, SessionId)>,
        generation: u64,
        session_id: SessionId,
    ) -> Result<FileInfo> {
        debug_assert_eq!(
            responses.len(),
            6,
            "stat compound must have 6 sub-responses"
        );
        ...
        if close_header.status != NtStatus::SUCCESS {
            if let Some(fid) = file_id {
                cleanup_handles.push((fid, generation, session_id));
            }
        }
    ...
    pub async fn rename_with_options(
        ...
        debug!(
            ...
            from_normalized, to_normalized, options.replace_if_exists
        );
    ...
    pub async fn rename_files(
        ...
        for (from, to) in renames {
            ...
            let setinfo_req = SetInfoRequest {
                ...
                additional_information: 0,
                file_id: FileId::SENTINEL,
                buffer: build_rename_info_buffer(&to_normalized, false),
            };
            let close_req = CloseRequest {
                flags: 0,
                file_id: FileId::SENTINEL,
